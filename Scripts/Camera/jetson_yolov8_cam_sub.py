...
class StereoViewerDetect(Node):
    def __init__(self) -> None:
        ...
        self.timer = self.create_timer(30 ** -1, self.draw)   # ~30 Hz
    ...
    def draw(self) -> None:
        ...
        l = cv2.flip(self.left_img[:h, :w], 0)  # flipCode=0 → flip vertical
        r = cv2.flip(self.right_img[:h, :w], 0)

        # Object detection on left image
        l = self.annotate(l)
        r = self.annotate(r)
        # Show
        cv2.imshow('Stereo + YOLO', np.hstack((l, r)))
        cv2.waitKey(1)
